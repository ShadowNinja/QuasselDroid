...
public final class CoreConnection {
	...
	public void requestUnhideTempHiddenBuffer(int bufferId) {
		...
		retFunc.add(new QVariant<Integer>(bufferId, "BufferId"));
		retFunc.add(new QVariant<Integer>(networks.get(buffers.get(
		                bufferId).getInfo().networkId).getBufferCount(), QVariantType.Int));
...
//			Log.e(TAG, "GETTING: "+buffers.get(buffer).getUnfilteredBacklogEntry(0).messageId);
			requestBacklog(buffer, -1, buffers.get(buffer).getUnfilteredBacklogEntry(0).messageId,
			               amount);
		}
	}
	...
	public void connect() throws UnknownHostException, IOException, GeneralSecurityException,
		CertificateException, NewCertificateException, EmptyQVariantException,
		...
		initial.put("ClientVersion",
		            new QVariant<String>("v0.6.1 (dist-<a href='http://git.quassel-irc.org/?p=quassel.git;a=commit;h=611ebccdb6a2a4a89cf1f565bee7e72bcad13ffb'>611ebcc</a>)",
		                                 QVariantType.String));
		...
		if (coreInfo.getProtocolVersion() < 10) {
			throw new UnsupportedProtocolException("Protocol version is old: " +
			                                       coreInfo.getProtocolVersion());
		}
		...
		if (ssl) {
			...
			SSLSocket sslSocket = (SSLSocket) sslSocketFactory.createSocket(socket, address, port,
			                      true);
		...
		Map<String, QVariant<?>> sessionState = (Map<String, QVariant<?>>)
		                                        reply.get("SessionState").getData();
		List<QVariant<?>> networkIds = (List<QVariant<?>>)
		                               sessionState.get("NetworkIds").getData();
		...
		List<QVariant<?>> bufferInfos = (List<QVariant<?>>)
		                                sessionState.get("BufferInfos").getData();
		...
		if (!options.getBoolean(service.getString(R.string.preference_fetch_to_last_seen),
		                        false)) {
			int backlogAmout = Integer.parseInt(options.getString(service.getString(
			                R.string.preference_initial_backlog_limit), "1"));
	...
	private void sendQVariantMap(Map<String, QVariant<?>> data) throws IOException {
		QVariant<Map<String, QVariant<?>>> bufstruct = new QVariant<Map<String, QVariant<?>>>
		(data, QVariantType.Map);
	...
	private void sendQVariantList(List<QVariant<?>> data) throws IOException {
		QVariant<List<QVariant<?>>> bufstruct = new QVariant<List<QVariant<?>>>(data,
		                QVariantType.List);
		sendQVariant(bufstruct);
	}
	...
	private Map<String, QVariant<?>> readQVariantMap() throws IOException,
		EmptyQVariantException {
		// Length of this packet (why do they send this? noone knows!).
		inStream.readUInt(32);
		QVariant<Map<String, QVariant<?>>> v = (QVariant<Map<String, QVariant<?>>>)
		                                       QMetaTypeRegistry.unserialize(QMetaType.Type.QVariant, inStream);
	...
	private List<QVariant<?>> readQVariantList() throws IOException, EmptyQVariantException {
		...
		QVariant<List<QVariant<?>>> v = (QVariant<List<QVariant<?>>>)
		                                QMetaTypeRegistry.unserialize(QMetaType.Type.QVariant, inStream);
	...
	private void sendInitRequest(String className, String objectName) throws IOException {
		List<QVariant<?>> packedFunc = new LinkedList<QVariant<?>>();
		packedFunc.add(new QVariant<Integer>(RequestType.InitRequest.getValue(),
		                                     QVariantType.Int));
		...
		public String doRun() throws EmptyQVariantException {
			...
			try {
				...
				if (e.getCause() instanceof NewCertificateException) {
					...
					service.getHandler().obtainMessage(R.id.NEW_CERTIFICATE,
					                                   ((NewCertificateException) e.getCause()).hashedCert()).sendToTarget();
					closeConnection();
				} else if (e.getCause() instanceof CertificateException) {
					Log.w(TAG, "Got CertificateException while connecting");
					service.getHandler().obtainMessage(R.id.INVALID_CERTIFICATE,
					                                   e.getCause().getMessage()).sendToTarget();
					closeConnection();
				} else {
					e.printStackTrace();
			...
			while (running) {
				try {
					...
					if (!networkInitComplete) {
						if (RequestType.getForVal((Integer) packedFunc.get(0).getData()) != RequestType.InitData
						                || !((String) packedFunc.get(1).getData()).equals("Network")) {
					...
					case HeartBeat:
						...
						packet.add(new QVariant<Integer>(RequestType.HeartBeatReply.getValue(),
						                                 QVariantType.Int));
					...
					case HeartBeatReply:
						...
						if (packedFunc.size() != 0) {
							...
							service.getHandler().obtainMessage(R.id.SET_CORE_LATENCY, latency, 0,
							                                   null).sendToTarget();
						}
						break;
						/*
					...
					case InitData:
						...
						if (className.equals("Network")) {
							...
							Map<String, QVariant<?>> initMap = (Map<String, QVariant<?>>) packedFunc.remove(
							                0).getData();
							...
							if (isConnected) {
								...
								network.setConnectionState(ConnectionState.Disconnected);
							}
							...
							Map<String, QVariant<?>> usersAndChans = (Map<String, QVariant<?>>)
							                initMap.get("IrcUsersAndChannels").getData();
							Map<String, QVariant<?>> channels = (Map<String, QVariant<?>>)
							                                    usersAndChans.get("channels").getData();

							//Parse out user objects for network
							Map<String, QVariant<?>> userObjs = (Map<String, QVariant<?>>)
							                                    usersAndChans.get("users").getData();
							...
							for (QVariant<?> channel : channels.values()) {
								...
								Map<String, QVariant<?>> userModes = (Map<String, QVariant<?>>)
								                                     chan.get("UserModes").getData();
							...
							List<QVariant<?>> lastSeen = (List<QVariant<?>>)((Map<String, QVariant<?>>)
							                             packedFunc.get(0).getData()).get("LastSeenMsg").getData();
							for (int i = 0; i < lastSeen.size(); i += 2) {
								...
								if (buffers.containsKey(bufferId)) { // We only care for buffers we have open
									if (PreferenceManager.getDefaultSharedPreferences(service).getBoolean(service.getString(
									                        R.string.preference_fetch_to_last_seen), false)) {
										initBacklogBuffers += 1;
							...
							QVariant<?> rawMarkerLines = ((Map<String, QVariant<?>>) packedFunc.get(
							                                      0).getData()).get("MarkerLines");
							...
							Map<String, QVariant<?>> userMap = (Map<String, QVariant<?>>) packedFunc.remove(
							                0).getData();
							...
							for (Buffer buffer : buffers.values()) {
								if (buffer.getInfo().name.equalsIgnoreCase(bufferName)
								                && buffer.getInfo().networkId == networkId) {
									found = true;
									Message msg = service.getHandler().obtainMessage(R.id.CHANNEL_TOPIC_CHANGED, networkId,
									                buffer.getInfo().id, topic);
									msg.sendToTarget();
									msg = service.getHandler().obtainMessage(R.id.SET_BUFFER_ACTIVE, buffer.getInfo().id, 0,
									                true);
									msg.sendToTarget();
									break;
								}
							...
							Map<String, QVariant<?>> map = (Map<String, QVariant<?>>) packedFunc.remove(0).getData();
							List<QVariant<?>> tempList = (List<QVariant<?>>)
							                             map.get("TemporarilyRemovedBuffers").getData();
					...
					case Sync:
						...
						if (className.equals("BacklogManager") && function.equals("receiveBacklog")) {
							...
							if (!initComplete) { //We are still initializing backlog for the first time
								boolean preferenceParseColors = PreferenceManager.getDefaultSharedPreferences(
								                                        service).getBoolean(service.getString(R.string.preference_colored_text), false);
								for (QVariant<?> message : data) {
									...
									if (!buffer.hasMessage(msg)) {
										 ...
										 */
										MessageUtil.checkMessageForHighlight(networks.get(buffer.getInfo().networkId).getNick(),
										                                     buffer, msg);
							...
							if (!initComplete) {
								...
								service.getHandler().obtainMessage(R.id.NEW_USER_ADDED, Integer.parseInt(objectName), 0,
								                                   user).sendToTarget();
							}
							...
							int networkId = Integer.parseInt(objectName);
							Network.ConnectionState state = ConnectionState.getForValue((Integer) packedFunc.remove(
							                                        0).getData());
							//If network has no status buffer it is the first time we are connecting to it
							if (state == ConnectionState.Connecting
							                && networks.get(networkId).getStatusBuffer() == null) {
								...
								service.getHandler().obtainMessage(R.id.SET_STATUS_BUFFER, networkId, 0,
								                                   buffer).sendToTarget();
							}
							service.getHandler().obtainMessage(R.id.SET_CONNECTION_STATE, networkId, 0,
							                                   state).sendToTarget();
							...
							int networkId = Integer.parseInt(objectName);
							service.getHandler().obtainMessage(R.id.SET_CONNECTED, networkId, 0,
							...
							int networkId = Integer.parseInt(objectName);
							service.getHandler().obtainMessage(R.id.SET_NETWORK_LATENCY, networkId, networkLatency,
							                                   null).sendToTarget();
							...
							service.getHandler().obtainMessage(R.id.SET_USER_SERVER, networkId, 0,
							                                   bundle).sendToTarget();
							...
							service.getHandler().obtainMessage(R.id.SET_USER_AWAY, networkId, 0,
							                                   bundle).sendToTarget();
							...
							service.getHandler().obtainMessage(R.id.SET_USER_REALNAME, networkId, 0,
							                                   bundle).sendToTarget();
							...
							service.getHandler().obtainMessage(R.id.USER_ADD_MODE, networkId, 0,
							                                   bundle).sendToTarget();
							...
							service.getHandler().obtainMessage(R.id.USER_REMOVE_MODE, networkId, 0,
							                                   bundle).sendToTarget();
							...
							if (buffers.containsKey(bufferId)) {
								...
								service.getHandler().obtainMessage(R.id.REMOVE_BUFFER, networkId,
								                                   bufferId).sendToTarget();
							}
...
//                                System.err.println("got buffer info for non-existant buffer id: " + bufferId);
							...
							msg.sendToTarget();
						                ...
						                && function.equals("removeBufferPermanently")) {
					...
					case RpcCall:
						...
						if (functionName.equals("2displayMsg(Message)")) {
							...
							if (!networks.get(message.bufferInfo.networkId).containsBuffer(message.bufferInfo.id) &&
								...
								Buffer buffer = new Buffer(message.bufferInfo,
								                           new QuasselDbHelper(service.getApplicationContext()));
							...
							if (message.type == IrcMessage.Type.NetsplitJoin) {
								...
								for (String nick : netsplitHelper.getNicks()) {
									IrcUser user = new IrcUser();
									user.nick = nick;
									service.getHandler().obtainMessage(R.id.NEW_USER_ADDED, message.bufferInfo.networkId, 0,
									                                   user).sendToTarget();
							...
							if (message.type == IrcMessage.Type.NetsplitQuit) {
								NetsplitHelper netsplitHelper = new NetsplitHelper(message.content.toString());
								for (String nick : netsplitHelper.getNicks()) {
									service.getHandler().obtainMessage(R.id.USER_QUIT, message.bufferInfo.networkId, 0,
									                                   nick).sendToTarget();
								}
							}
						...
						} else if (functionName.equals("__objectRenamed__")
						                && ((String) packedFunc.get(0).getData()).equals("IrcUser")) {
							...
							service.getHandler().obtainMessage(R.id.USER_CHANGEDNICK, networkId, -1,
							                                   bundle).sendToTarget();
					...
					if (end - start > 500) {
						System.err.println("Slow parsing (" + (end - start) + "ms)!: Request type: " + type.name()
