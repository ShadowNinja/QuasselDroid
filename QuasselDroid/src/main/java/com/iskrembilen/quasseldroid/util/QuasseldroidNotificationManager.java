...
public class QuasseldroidNotificationManager {
	...
	private void notifyConnected(boolean withPhysicalNotifications) {
		...
		if (withPhysicalNotifications
		                && preferences.getBoolean(context.getString(R.string.preference_notify_connect), false)) {
			if (preferences.getBoolean(context.getString(R.string.preference_notification_vibrate),
			                           true)) {
				defaults |= Notification.DEFAULT_VIBRATE;
			}

			if (preferences.getBoolean(context.getString(
			                                   R.string.preference_notification_sound_active), false) &&
			                preferences.getBoolean(context.getString(R.string.has_focus), true) == false &&
			                preferences.getBoolean(context.getString(R.string.preference_notification_sound),
			                                       false)) {
				Uri ringtone = Uri.parse(preferences.getString(context.getString(
				                                 R.string.preference_notification_connect_sound_file), ""));
				if (ringtone.equals(Uri.EMPTY)) {
					defaults |= Notification.DEFAULT_SOUND;
				} else {
					builder.setSound(ringtone);
				}
			} else if (preferences.getBoolean(context.getString(
			                R.string.preference_notification_sound_active), true) == false &&
			                preferences.getBoolean(context.getString(R.string.preference_notification_sound),
			                                       false)) {
				Uri ringtone = Uri.parse(preferences.getString(context.getString(
				                                 R.string.preference_notification_connect_sound_file), ""));
	...
	public void notifyHighlight(Integer bufferId) {
		...
		.setContentText(context.getResources().getQuantityString(
		                        R.plurals.notification_highlighted_on_x_buffers, highlightedBuffers.size(),
		                        highlightedBuffers.size()))
		.setOngoing(true)
		...
		if (bufferId != null) {
			if (preferences.getBoolean(context.getString(
				...
				Uri ringtone = Uri.parse(preferences.getString(context.getString(
				                                 R.string.preference_notification_sound_file), ""));
				if (ringtone.equals(Uri.EMPTY)) {
					defaults |= Notification.DEFAULT_SOUND;
				} else {
					builder.setSound(ringtone);
			...
			if (preferences.getBoolean(context.getString(R.string.preference_notification_light),
			                           false)) {
